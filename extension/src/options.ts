...
function hasValidConfig(settings: AppSettings): boolean {
  return settings.proxies.some(p => p.hosts && p.hosts.length > 0);
}

function renderProxyMode(): void {
  if (!currentSettings) return;
  const hasConfig = hasValidConfig(currentSettings);
  const mode = currentSettings.mode || (currentSettings.enabled ? 'by_rule' : 'off');

  const warningEl = document.getElementById('no-proxy-warning');
  if (warningEl) {
    warningEl.style.display = hasConfig ? 'none' : 'flex';
  }

  // Disable by_rule and global mode options when no proxy servers are configured
  radioModeByRule.disabled = !hasConfig;
  radioModeGlobal.disabled = !hasConfig;

  // Add disabled styles / cursor to labels if disabled
  const byRuleLabel = radioModeByRule.closest('label');
  const globalLabel = radioModeGlobal.closest('label');
  if (byRuleLabel) {
    byRuleLabel.style.opacity = hasConfig ? '1' : '0.5';
    byRuleLabel.style.cursor = hasConfig ? 'pointer' : 'not-allowed';
  }
  if (globalLabel) {
    globalLabel.style.opacity = hasConfig ? '1' : '0.5';
    globalLabel.style.cursor = hasConfig ? 'pointer' : 'not-allowed';
  }

  // If no config, automatically enforce OFF mode if previously enabled
  if (!hasConfig && mode !== 'off') {
    applySettings({ mode: 'off', enabled: false });
    radioModeOff.checked = true;
    radioModeByRule.checked = false;
    radioModeGlobal.checked = false;
    globalPolicyContainer.style.display = 'none';
    return;
  }
