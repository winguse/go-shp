...
function renderMiniLatencyChart(): void {
  ...
  if (allValues.length > 0) {
    const p75Index = Math.min(
      allValues.length - 1,
      Math.floor(allValues.length * 0.75)
    );
    maxLatency = allValues[p75Index] * 2;
  }
...
async function toggleProxy(): Promise<void> {
  ...
  // If proxy is currently off and user has no proxy server hosts configured, do not enable proxy and navigate to options page
  if (currentMode === 'off' && !hasValidConfig(currentSettings)) {
    openOptions();
    return;
  }
