/**
 * storage.ts — Unified cross-browser storage helpers and robust YAML import/export.
 */

import type { AppSettings, HostLatency, ProxyGroup, RoutingRule, UnmatchedPolicy } from './types';

// Polyfill / extension API compatibility reference
...
export async function getSettings(): Promise<AppSettings> {
  const data = await extApi.storage.local.get('settings');
...
export async function saveSettings(settings: AppSettings): Promise<void> {
  ...
  extApi.runtime.sendMessage({ type: 'SETTINGS_UPDATED', settings }).catch(() => {});
}

// ── Latency storage ───────────────────────────────────────────────────

export async function getLatencies(): Promise<Record<string, HostLatency>> {
  const data = await extApi.storage.local.get('latencies');
...
export async function saveLatencies(latencies: Record<string, HostLatency>): Promise<void> {
  await extApi.storage.local.set({ latencies });
}
